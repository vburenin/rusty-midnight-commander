...
use rmc_fs::sftpfs::{set_host_key_action, set_known_hosts_path, HostKeyAction};
use rmc_fs::Vfs;
...
#[test]
fn fish_list_stat_copy_out_and_parent_leave() {
    ...
    let enter = vfs.enter_path(&ssh.sh_url("")).expect("sh enterable");
    assert_eq!(
        enter,
        PathBuf::from(format!("/#sh:user@127.0.0.1:{}", ssh.port))
    );
    ...
    assert_eq!(
        pub_ent.path,
        PathBuf::from(format!("/#sh:user@127.0.0.1:{}/pub", ssh.port))
    );
...
#[test]
fn fish_list_via_sh_url_and_hash_sh_prefix() {
    let _serial = lock_host_keys();
    let _kh = prepare_known_hosts();
    let ssh = FakeSsh::spawn(fixture_tree());
    let vfs = CompositeFs::new();
    ...
    let hash = PathBuf::from(format!("/#sh:user@127.0.0.1:{}", ssh.port));
    let list2 = vfs.list_dir(&hash, false).unwrap();
    assert!(names(&list2).contains(&"pub".into()));
}

#[test]
fn fish_connect_error_is_plain_fserror() {
    let vfs = CompositeFs::new();
    vfs.set_dir_cache_timeout_secs(0);
    let err = vfs
        .list_dir(Path::new("sh://127.0.0.1:1/"), false)
        .unwrap_err();
    let msg = err.to_string();
    assert!(
        msg.contains("SSH") || msg.contains("FISH") || msg.contains("connect"),
        "error should be a plain FsError for the existing dialog, got {msg}"
    );
}

#[test]
fn fish_nested_parent_then_leave() {
    let _serial = lock_host_keys();
    let _kh = prepare_known_hosts();
    let ssh = FakeSsh::spawn(fixture_tree());
    let vfs = CompositeFs::new();
...
#[test]
fn fish_url_includes_password_in_sftp_style_but_panel_omits_pass() {
    // sh://user:pass@host is parsed; FISH panel authority is user@host (no password).
    let u = rmc_fs::remote::parse_remote_url_str("sh://alice:secret@h:2222/d").unwrap();
